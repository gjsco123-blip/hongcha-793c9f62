...
━━━ [최우선] 문장 마무리 규칙 ━━━
모든 문장은 명사형으로 마무리해. 서술형 종결 어미를 쓰지 마.

✅ 허용되는 명사형 마무리 (예시):
~경향이 강함, ~라는 전제, ~라는 의미, ~라는 관점, ~로 이어짐,
~하는 구조, ~하는 흐름, ~하는 방식, ~라는 점, ~라는 차이,
~라는 결과, ~에 해당, ~과 같은 맥락, ~으로의 전환, ~라는 뜻
→ 위 예시에 준하는 자연스러운 명사형 마무리도 가능

❌ 절대 금지 마무리:
~이다, ~한다, ~라고 말한다, ~의미이다, ~것이다,
~된다, ~있다(종결), ~나타낸다, ~보여준다, ~설명한다
❌ "이 문장은~", "해당 문장은~" 같은 딱딱한 시작 금지
❌ 첫 문장을 "즉," "다시 말해,"로 시작하는 것 금지
...
• 추상적인 개념이 나오면, 학생이 이해할 수 있도록 그 개념이 실제로 무엇을 뜻하는지 구체적인 말로 풀어 설명
• 예: exclusionary citizenship → 시민만 중요한 정치적 권리를 가지는 구조 / active democracy → 실제로 공동체에 참여하는 사람들이 함께 결정하는 민주주의
• 괄호 보충 1회까지 허용
...
→ 왜 틀림: "~의미한다" 서술형 종결, "이 문장은~" 시작, 딱딱함

✅ 올바른 출력 예시:
"훈련이나 경기 중 숨이 자연스럽게 빨라지고, 그 결과 몸이 긴장하는 쪽(교감신경)이 활성화되는 구조. 즉, 몸이 '전투 준비'처럼 각성 상태로 전환되는 흐름."`;
...
const fewShotExamples = [
  ...
  },
  {
    ...
    content: `글에서 읽은 단어를 '의미'로 바꾸어 이해하면서 머릿속 세계를 채워 나가는 과정. 이때 떠오르는 장면은 새로 만들어지는 것이 아니라, 기존 기억에서 꺼내져 구성되는 방식.`,
  },
  {
    role: "user" as const,
  ...
  {
    role: "assistant" as const,
    content: `훈련이나 경기 중 숨이 자연스럽게 빨라지고, 그 결과 몸이 긴장하는 쪽(교감신경)이 활성화되는 구조. 즉, 몸이 '전투 준비'처럼 각성 상태로 전환되는 흐름.`,
  },
  {
    role: "user" as const,
  ...
  {
    role: "assistant" as const,
    content: `그림이 미술관에 걸려 있으면 사람들이 '분명 의미가 있을 것'이라 여기는 경향이 강함. 같은 그림이라도 장소가 미술관이면 더 진지하게 해석하려는 방향으로 이어짐.`,
  },
  {
    role: "user" as const,
  ...
  {
    role: "assistant" as const,
    content: `예술 작품을 볼 때 그 작품이 놓인 장소에 따라 해석이 달라진다는 관점. 미술관 같은 공간에 있으면 더 깊은 의미가 있을 거라 생각하게 되는 경향이 강함.`,
  },
];
