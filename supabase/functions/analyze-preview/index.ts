...
- 한국 중학생도 바로 이해할 수 있는 쉽고 자연스러운 한국어를 사용할 것.
- 어려운 한자어식 표현, 지나치게 학술적인 명사 표현, 교과서식 압축 표현은 피할 것.
- 같은 뜻이면 더 쉽고 풀어서 쓴 표현을 우선할 것.
- 예를 들어 "~의 필요성"보다는 "~가 왜 필요한지", "~의 상관관계"보다는 "~가 서로 어떤 관련이 있는지", "~를 도모"보다는 "~를 돕기 위해", "~를 함양"보다는 "~를 기르기"처럼 쓸 것.
- 문장을 작성한 뒤, 중학생이 바로 뜻을 설명하기 어려운 표현이 있으면 더 쉬운 말로 고쳐서 출력할 것.

[종결 스타일 — 명사형만 허용]
- 허용: ~라는 점, ~하는 구조, ~하는 흐름, ~라는 전제, ~경향, ~라는 의미, ~하는 방식, ~필요성, ~중요성, ~라는 주장
- 금지: ~한다, ~된다, ~이다, ~있다, ~했다, ~합니다, ~됩니다, ~임, ~함
- 명사형 종결을 유지하되, 지나치게 딱딱한 개념어 대신 쉬운 풀이형 표현을 사용할 수 있다.
- 예: "~하는 경향"뿐 아니라 "~하는 모습", "~가 커지는 흐름", "~가 왜 중요한지" 같은 표현도 허용한다.
