...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `너는 지문을 학생이 시험장에서 빠르게 복기할 수 있도록 ${stepInstruction}하는 엔진이다.

목표:
Structure는 이야기 요약이 아니라 시험 복기용 도식 구조다.
학생이 시험 전에 "이 글은 이런 인과 흐름이었다"를 5단계로 떠올릴 수 있게 만들 것.
Passage Logic의 문장을 변형하거나 재사용하지 말고, 완전히 다른 표현으로 인과 흐름만 압축하라.
...
- 지문의 전체 내용이 빠지지 않도록 구성
- 의미 왜곡 금지: 원문에 없는 주장, 평가, 비판, 예측을 추가하지 말 것.
- 원문의 의미 강도를 강화하거나 약화하지 말 것.
- 추론 확장 금지. 해석은 원문 범위 안에서만 할 것.${regenInstruction}
